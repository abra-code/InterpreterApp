...
# bundles) to Contents/Support/MLX/, build + embed pdfutil from its sibling repo, (3) download
# the llama.cpp release into Contents/Support/Llama.cpp/ (--skip-llama leaves it untouched),
# (4) deep-sign the whole bundle via codesign_applet.sh, (5) verify the deployed agent launches
# and is the new build (its usage lists `map`) and pdfutil reports its version.
#
...
DO_BUILD="yes"
DO_CODESIGN="yes"
# llama.cpp engine (gguf models): provisioned on every run, tracking upstream's newest official
# release. --skip-llama leaves Contents/Support/Llama.cpp untouched.
DO_LLAMA="yes"
LLAMA_VERSION="latest"
# Set by --llama-version, so that pairing it with --skip-llama can be refused rather than
# silently honoring one of the two.
LLAMA_VERSION_EXPLICIT="no"
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        # Back-compat: llama.cpp is provisioned by default now, so this asks for the default.
        # Kept accepted rather than removed so existing callers do not die on "Unknown option".
        --with-llama) DO_LLAMA="yes" ;;
        --llama-version=*) LLAMA_VERSION="${1#*=}"; LLAMA_VERSION_EXPLICIT="yes" ;;
        --help)
            echo "Usage: $0 [--release] [--arch=arm64|x86_64] [--agent-repo=PATH] [--skip-build] [--identity=CERT] [--no-codesign] [--skip-llama] [--llama-version=bNNNN|latest|nightly]"
            echo "  --skip-llama     leave Contents/Support/Llama.cpp untouched (llama.cpp is provisioned by default)"
            echo "  --llama-version  which llama.cpp build to deploy. Takes:"
            echo "                   latest   upstream's newest official release, resolved to the"
            echo "                            build carrying the binaries (the default)"
            echo "                   nightly  upstream's newest build, released or not"
            echo "                   bNNNN    an explicit build tag"
            exit 0 ;;
...
resolve_llama_version() {
    ...
    if [ "$LLAMA_VERSION" = "nightly" ]; then
        ...
        [ -n "$_tag" ] \
            || fail "No bNNNN tag in the llama.cpp releases list - github.com unreachable, or the API rate-limited this host. Pass --llama-version=bNNNN explicitly, or --skip-llama to leave the deployed engine untouched."
        LLAMA_VERSION="$_tag"
    ...
    [ -n "$_release" ] \
        || fail "Could not read a tag from github.com/ggml-org/llama.cpp/releases/latest - unreachable, or the API rate-limited this host. Pass --llama-version=bNNNN explicitly, or --skip-llama to leave the deployed engine untouched."
    ...
    [ -n "$_tag" ] \
        || fail "Release $_release has no readable nightly-tag.txt, so the build tag carrying the macOS binaries is unknown - the release scheme has changed again. Check https://github.com/ggml-org/llama.cpp/releases and pass --llama-version=bNNNN, --llama-version=nightly for the newest build, or --skip-llama to leave the deployed engine untouched."
    LLAMA_VERSION="$_tag"
...
# --skip-llama skips the stage that would consume --llama-version, so honoring one of the two
# means quietly ignoring the other. Refuse instead: the caller asked for two different things.
[ "$DO_LLAMA" = "yes" ] || [ "$LLAMA_VERSION_EXPLICIT" = "no" ] \
    || fail "--skip-llama cannot be combined with --llama-version=$LLAMA_VERSION: the llama.cpp stage does not run, so the version would be ignored."
...
# Settle which llama.cpp build to deploy before the build steps, not at the download: this is
# a network round-trip on every default run, and failing after a multi-minute xcodebuild would
# leave the bundle freshly deployed but unsigned and unverified. update-cadabra.sh resolves in
# prepare() for the same reason.
if [ "$DO_LLAMA" = "yes" ]; then
    # Whether this block printed anything, so the separating blank line below is not emitted
    # into silence
    _lspoke="no"
    case "$LLAMA_VERSION" in
        latest|nightly) resolve_llama_version; _lspoke="yes" ;;
    esac
...
# ── 2c. llama.cpp engine (gguf models, on by default) ─────────────────────
# Prebuilt upstream release tarball -> Contents/Support/Llama.cpp/ (llama-server + dylibs,
# @rpath-linked so they only need to sit together). Same provisioning as AIChat V2: on by
# default, tracking the newest official release, with --skip-llama to leave it alone.
#
LLAMA_DIR="$APP_BUNDLE/Contents/Support/Llama.cpp"
if [ "$DO_LLAMA" = "yes" ]; then
    # LLAMA_VERSION was resolved and validated up front, right after the banner.
    ...
    /usr/bin/curl -L --fail --show-error --progress-bar -o "$_lwork/$_lasset" \
        ...
        || fail "llama.cpp download failed - github.com unreachable? --skip-llama leaves the deployed engine untouched."
    ...
    [ -n "$_lfound" ] && [ -f "$_lfound" ] || { /bin/rm -rf "$LLAMA_DIR"; fail "No LICENSE in the llama.cpp $LLAMA_VERSION archive - the MIT notice has to ship beside llama-server. Contents/Support/Llama.cpp has been removed; re-run once the archive layout is sorted out."; }
...
if [ -n "$(/usr/bin/find "$LLAMA_DIR" ! -type d ! -name LICENSE -print -quit 2>/dev/null)" ]; then
    require_license "$LLAMA_DIR" "$LLAMA_DIR/LICENSE" "a full run (no --skip-llama)" "the llama.cpp engine"
fi
