# interp.from.changed - persist the From-language selection. The picker delivers a 1-based
# index into the CURRENT (possibly family-filtered) option list; persist the language CODE it
...
[ -n "$code" ] && /usr/bin/defaults write "$BUNDLE_ID" FromLang "$code"
